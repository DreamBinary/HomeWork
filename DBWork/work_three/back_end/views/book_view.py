    ...
    if user is None:
        ...
        for book in books:
            data["self"].append({
                ...
                "multiuser": [User.query.filter_by(id=multiuser.user_id).first().username for multiuser in
                              book.multiuser]
            })
        ...
        for multiuser_user in multiusers_user:
            ...
            a_multiuser_book = {
                ...
                "update_time": book.update_time,
                "description": book.description,
                "multiuser": [User.query.filter_by(id=multiuser_book.user_id).first().username for multiuser_book in
                              book.multiuser]
            }
...
def delete():
    ...
    book = Book.query.filter_by(id=book_id).first()
    if book is None:
        return Response(404, "账本不存在").to_json()
    else:
